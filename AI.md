...
There is not yet a story formatter, full static type checker, save format, RPG behavior module, full desktop editor, native shell, or exporter. Empty entry points outside the runnable path still establish buildable boundaries only and must not be described as implemented product features.
...
2. Build shared export orchestration for the static web target and wire it to both CLI and GUI entry points.
3. Move the working playground surfaces into the Tauri editor shell with filesystem-backed project open/save and source conflict handling.
