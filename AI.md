...
- `apps/editor` is now a working Tauri 2 shell around the exact `apps/playground` Creator Studio frontend; it does not maintain a second authoring implementation. The Rust host owns canonical project roots behind opaque session IDs. Its custom commands open a native folder picker, read bounded UTF-8 `project.toml`/ordinary `.story`/optional `.rpgne/editor.json` files without following symlink components, reload the authorized root, compare content revisions, save only files opened in that session, write the shared build service's output, and open only the last successfully written output folder. The webview receives no general path-based filesystem or shell command; the sole capability grants Tauri core APIs to the main window.
- Native project saves preflight the complete current disk snapshot against the session baseline before writing, so an added, removed, or edited source blocks the save and names the changed paths. Valid saves stage every requested source beside its destination, move existing files to transaction-specific backups, promote every staged file, and roll back ordinary operation failures before reporting success. The frontend tracks a full in-memory fingerprint, exposes Saved/Unsaved/Changed-on-disk/error state plus Save and Reload controls, supports Ctrl/Cmd+S, protects project replacement/window close, and rechecks disk state on focus and immediately before save. A continuously running filesystem watcher and recovery of process/power loss during the brief multi-file promotion window remain pending.
- Native GUI builds still call `buildWebProject` in the shared frontend. The Rust adapter validates canonical artifact paths and the artifact manifest's project ID, rejects reserved/symlinked output paths, refuses to replace nonempty unowned directories, uses a project-local exclusive lock plus owned staging/previous directories, promotes output by same-filesystem rename, and retains the authorized successful path for the GUI **Open folder** action. Tauri bundling is intentionally disabled for now: `pnpm editor:build` compiles the current-host executable, while installer/signing work remains later.
- Scratch mode locally persists/imports/downloads one story. Browser project mode accepts a directory, preserves manifest/story/editor-metadata buffers, compiles all selected story files, and exposes the GUI web Build dialog, but browser directory access remains read-only: Download file exports the active story buffer and Download layout exports `.rpgne/editor.json`. Undo/redo remains pending in both hosts.
...
- One fast Linux GitHub Actions verification job. The Tauri shell is compiled locally during its implementation; do not add an operating-system matrix merely because the shell exists. Add focused platform jobs only when installer/export artifacts create behavior worth exercising there.
...
There is not yet a story formatter, full static type checker, save migration/auto-save/transcript system, RPG behavior module, undo/history system, continuous filesystem watcher, PWA exporter, desktop installer, or native game exporter. Empty entry points outside the runnable path still establish buildable boundaries only and must not be described as implemented product features.
...
- Desktop host: Tauri 2.11.5 with exact Rust/plugin dependencies and a committed Cargo lockfile. The non-production application identifier is `org.rpgnarrativeengine.editor.dev`; installer bundling remains disabled.
...
1. Add creator undo/redo history across story, manifest, and editor-metadata transactions, with native saves as the clean-history boundary.
2. Add continuous native filesystem watching and interrupted-save recovery on top of the current focus/pre-save conflict checks and rollback path.
...
5. Continue command schemas, static typing, formatting, localization, modules, desktop installers, and native game targets as those working paths require them.
