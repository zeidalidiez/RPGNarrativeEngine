...
- `apps/playground` is now the browser-runnable beginning of the GUI-first creator application rather than a source-editor prototype. It opens in a structured Scene Builder with a project-wide, file-grouped scene navigator and ordered cards for narration, dialogue, choices, conditions, commands/state changes, and endings. Creators can add scenes and common card types, edit their labeled fields and project-aware choice destinations, switch narration/dialogue modes, preserve content IDs, move, duplicate, remove, and save cards, and see the production player recompile immediately. Every committed card operation is applied to the same canonical `.story` buffer through `@rpgnarrativeengine/editor-source`; Advanced Source is an optional tab and diagnostics can still select exact ranges there. Nested conditional branches are currently preserved and summarized but still require Advanced Source until their structured condition builder is implemented. Scratch mode locally persists/imports/downloads one story. Project mode accepts a directory, preserves manifest/story buffers, compiles all selected story files, and exposes the existing GUI web Build dialog. Browser directory access remains read-only and Download file exports only the active source buffer; filesystem-backed project save belongs to the Tauri host.
...
- `editor-source`: currently parses canonical story buffers for structured controls and applies one bounded UTF-16 source replacement atomically, rejecting edits whose resulting story cannot normalize. Multi-file transactions, semantic compiler validation, undo history, TOML edits, and external-change conflict handling remain pending.
...
1. Extend structured scene cards to nested conditions/actions, add scene rename/reference transactions, and build the directly editable Story Map over the same canonical buffers.
2. Move the working creator and Build surfaces into the Tauri editor shell with filesystem-backed project open/save, undo/redo, output-folder actions, and source conflict handling.
