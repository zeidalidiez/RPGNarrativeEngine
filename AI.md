...
- `apps/playground` is now the browser-runnable beginning of the GUI-first creator application rather than a source-editor prototype. It opens in a structured Scene Builder with a project-wide, file-grouped scene navigator and ordered cards for narration, dialogue, choices, conditions, commands/state changes, and endings. Creators can add scenes and card types, edit labeled fields and project-aware destinations, switch narration/dialogue modes, preserve content IDs, move, duplicate, remove, and save cards, and see the production player recompile immediately. Guided condition controls cover booleans, text/number/variable equality, numeric comparisons, and an advanced-expression escape hatch. Conditional cards expose true/otherwise branch panels whose nested narration, dialogue, choice, state, condition, and ending cards can be edited or added without writing syntax. The editable Story Map derives nodes and choice/`@goto`/`@call` edges from the same AST, lays them out deterministically, creates scenes and connections, retargets or removes root connections, renames scene declarations and all story references across files, and opens a node directly in the Scene Builder. If a renamed scene is the project entry, a formatting-preserving TOML transaction updates `[project].entry_scene` before any in-memory file is committed. Every visual operation writes the same canonical `.story` buffers through validated source transactions; Advanced Source is an optional tab and diagnostics can still select exact ranges there. Scratch mode locally persists/imports/downloads one story. Project mode accepts a directory, preserves manifest/story buffers, compiles all selected story files, and exposes the existing GUI web Build dialog. Browser directory access remains read-only and Download file exports only the active source buffer; filesystem-backed project save, undo/redo, draggable persisted map layout, grouped AND/OR condition construction, nested connection deletion, and scene deletion/duplication belong to later editor work.
...
- `editor-source`: parses canonical story buffers for structured controls and applies one or several non-overlapping UTF-16 source replacements as one parse-validated transaction. It also performs project-wide scene renames by locating the one declaration plus choice, `@goto`, and `@call` references recursively through conditional branches, validating every resulting story file before returning any updates. The creator host separately composes the matching formatting-preserving `project.toml` entry-scene update into the same in-memory commit. Semantic compiler validation, general-purpose multi-file transactions, undo history, broader TOML edits, and external-change conflict handling remain pending.
...
1. Add scene deletion/duplication transactions, grouped AND/OR condition construction, nested connection removal, and persistent creator-owned Story Map positioning.
2. Move the working creator and Build surfaces into the Tauri 2 editor shell with filesystem-backed project open/save, undo/redo, output-folder actions, and source conflict handling.
