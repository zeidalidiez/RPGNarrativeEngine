...
- C-02 syntax is fixed in `docs/contracts/story-language.md` and Build Plan sections 11.8-11.9. `@rpgnarrativeengine/language` implements variable-path, finite-number, exact-duration, and quoted-string lexical primitives plus the shared Lezer grammar, generated parser, indentation/context tokenizer, recoverable CST, and source-ranged character/indentation/parse issues. Valid files normalize into a deeply immutable structural AST for scenes, trivia, text beats/continuations, generic registered commands, conditionals, and choices. Normalization validates stable IDs and block markers, rebases embedded choice/`@if` expressions to file coordinates, separates inline comments and content IDs from rendered text, and rejects shorthand-target/body conflicts and nested v1 choices. The complete, multiline, malformed, and targeted semantic-rejection cases are exercised by tests. Safe-markup/dialogue normalization, command-argument semantics, and the formatter remain pending.
...
- `language`: owns the story/expression grammar, CST/AST, formatting, and language services. It currently implements lexical contract helpers, executable expression primitives/metadata, the shared generated Lezer CST parser, the normalized expression AST/source mapper, and the structural story AST. Inline text/markup and command semantics, formatting, and higher language services remain pending.
...
1. Complete C-02 AST normalization for safe text/dialogue and command arguments, then implement the idempotent formatter against the committed corpus.
