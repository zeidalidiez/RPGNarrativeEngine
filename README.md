...
The first complete playable path now works. A creator `project.toml` is validated into a typed manifest, its `[story].files` globs safely discover canonically ordered story sources, and those files compile together into a serializable, platform-neutral instruction format with project-wide scene/reference validation. The headless runtime executes state changes, expressions, branches, calls, choices, effects, scene transitions, and endings; and the browser player renders the result with safe semantic DOM. `random()` expressions and future module mechanics use independent, deterministic named random streams rather than `Math.random`. Save-enabled web exports include visible **Save game** and **Load game** controls backed by a versioned runtime snapshot, including exact random-stream continuation. The creator app now opens in a visual **Scene Builder**, not the raw text editor: its project-wide scene navigator and structured cards can create scenes, add and edit narration/dialogue/choices/state/actions/endings, select destinations, reorder or duplicate cards, and immediately preview the canonical story. **Advanced Source** remains available as an optional escape hatch while structured conditional branches and the editable Story Map are completed. `templates/first-story` is a working two-file starter project, and `examples/showcase` contains the GitHub Pages reference story. The GUI Build workspace packages an opened project as a deterministic static-web ZIP and/or self-contained HTML; the matching `rpgne build` command writes the bundle, web folder, ZIP/HTML, checksums, manifest, and report into an atomically promoted project output directory. Full structured condition editing, editable Story Map, filesystem-backed Tauri authoring, save migration/auto-save/history, PWA output, and native Tauri game packages remain to be built.
