...
The first complete playable path now works. A creator `project.toml` is validated into a typed manifest, its `[story].files` globs safely discover canonically ordered story sources, and those files compile together into a serializable, platform-neutral instruction format with project-wide scene/reference validation. The headless runtime executes state changes, expressions, branches, calls, choices, effects, scene transitions, and endings; and the browser player renders the result with safe semantic DOM. Save-enabled web exports now include visible **Save game** and **Load game** controls backed by a versioned runtime snapshot: variables, nested instruction location, call stack, current view, and pending choices resume without repeating effects, while saves from another bundle build are rejected without replacing the live state. `templates/first-story` is a working two-file starter project. `examples/showcase` contains the manifest-backed branching story **The Light at Brinewatch**, which can be built as a static GitHub Pages site. `apps/playground` provides a creator-facing browser workspace that opens either a standalone `.story` file or a complete project directory, switches among manifest and story buffers, compiles all selected story files together, routes diagnostics back to the correct file, and runs the result without a terminal. Its **Build project** workspace packages an opened project as a deterministic static-web ZIP and/or a self-contained HTML file, with a shared bundle hash and downloadable artifact manifest. The matching `rpgne build` command writes the same bundle, web folder, ZIP/HTML, checksums, manifest, and report into an atomically promoted project output directory. Named RNG, save migration/auto-save/history, full desktop filesystem authoring, PWA output, and native Tauri game packages remain to be built.
